      ...
      <div className="max-w-6xl w-full max-h-[90vh] flex flex-col bg-gray-900 rounded-lg overflow-hidden">
        ...
        <div className="flex items-center justify-between p-4 border-b border-gray-700">
          ...
          <div className="flex gap-2">
            ...
            <a
              ...
              className="px-3 py-2 bg-brand-primary text-white rounded-lg hover:bg-opacity-90 transition-colors text-sm"
            >
              Download
            </a>
            <button
        ...
        <div className="flex-1 overflow-auto p-4">
          <div className="flex gap-6">
            {/* Preview */}
            <div className="flex-1 flex items-center justify-center bg-gray-800 rounded-lg min-h-[400px]">
              {isImage && (
                <img
                  src={asset.fileUrl}
                  alt={asset.fileName}
              ...
              {isPDF && (
                <div className="text-center p-8">
                  ...
                  <a
                    ...
                    className="px-4 py-2 bg-brand-primary text-white rounded-lg hover:bg-opacity-90 transition-colors inline-block"
                  >
                    Open PDF in New Tab
                  </a>
                </div>
            ...
            <div className="w-80 space-y-4">
              ...
              <div className="bg-gray-800 rounded-lg p-4 border border-gray-700">
                ...
                <div className="space-y-2 text-sm">
                  <div>
                    ...
                    <span className="text-white ml-2">{asset.fileType}</span>
                  </div>
                  <div>
                    <span className="text-gray-400">Size:</span>
                    <span className="text-white ml-2">{formatFileSize(asset.fileSize)}</span>
                  </div>
                  <div>
                    <span className="text-gray-400">Uploaded:</span>
                    <span className="text-white ml-2">{formatDate(asset.uploadedAt)}</span>
                  </div>
                  <div>
                    <span className="text-gray-400">Uploaded By:</span>
                    <span className="text-white ml-2">{asset.uploadedBy}</span>
                  </div>
                  <div>
                    <span className="text-gray-400">Category:</span>
                    <span className="text-white ml-2 capitalize">{asset.category.replace('-', ' ')}</span>
                  </div>
                </div>
              </div>
                  ...
                  <div className="space-y-3 text-sm">
                    {asset.metadata.description && (
                      <div>
                        ...
                        <span className="text-white">{asset.metadata.description}</span>
                      </div>
                    )}
                    ...
                    {asset.metadata.campaignName && (
                      <div>
                        <span className="text-gray-400 block mb-1">Campaign:</span>
                        <span className="text-white">{asset.metadata.campaignName}</span>
                      </div>
                    )}
              ...
              <div className="bg-gray-800 rounded-lg p-4 border border-gray-700">
                ...
                <div className="space-y-2">
                  <button
                    onClick={() => onEdit(asset.id)}
                    className="w-full px-3 py-2 bg-gray-700 text-white rounded-lg hover:bg-gray-600 transition-colors text-sm flex items-center justify-center gap-2"
                  >
